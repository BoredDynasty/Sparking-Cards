...
local function RegisterError(error_message, store_name, profile_key) -- Called when a DataStore API call errors
	warn(
		`[{script.Name}]: DataStore API error (STORE:{store_name}; KEY:{profile_key}) - {tostring(
			error_message
		)}`
	)
...
local function MockUpdateAsync(mock_data_store, profile_store_name, key, transform_function, is_get_call) --> loaded_data, key_info
	...
	local transform, user_ids, roblox_meta_data =
		transform_function(mock_entry and mock_entry.Data, mock_key_info)
		...
		elseif DataStoreState ~= "Access" then -- Used when API access is disabled
			...
			if is_get_call == true then
				if version ~= nil then
					...
					then
						warn(
							`[{script.Name}]: Passed version argument is not valid; Traceback:\n`
								.. debug.traceback()
						)
					end
				else
...
function ProfileStore:StartSessionAsync(profile_key, params)
	...
	if ActiveSessionCheck[session_token] ~= nil then
		error(
			`[{script.Name}]: Profile (STORE:{self.Name}; KEY:{profile_key}) is already loaded in this session`
		)
	end
			...
			profile_load_job[2] = table.pack(UpdateAsync(self, profile_key, {
				...
				MissingProfileHandle = function(latest_data)
					...
					latest_data.MetaData = {
						...
						SessionLoadCount = 0,
						ActiveSession = if is_cancel == false
							then { PlaceId, JobId, unique_session_id }
							else nil,
		...
		if loaded_data ~= nil and key_info ~= nil then
			...
			if type(active_session) == "table" then
				if IsThisSession(active_session) == true then
					...
					local profile =
						Profile.New(loaded_data, key_info, self, profile_key, is_mock, session_token)
